...
final class AdminController extends Controller
{
    ...
    public function updateOrder(Request $request,string $id): never
    {
        ...
        try{
            $statement=$pdo->prepare('SELECT order_status,email,order_number FROM orders WHERE id=? FOR UPDATE');$statement->execute([(int)$id]);$order=$statement->fetch();$new=(string)$request->input('status','');
            ...
            if($request->input('notify')) $pdo->prepare("INSERT INTO email_queue (template_key,recipient,subject,payload_json,status,next_attempt_at) VALUES ('order_status',?,?,?,'pending',NOW())")->execute([$order['email'],'Actualizare comandă '.$order['order_number'],json_encode(['status'=>$new,'message'=>$message],JSON_UNESCAPED_UNICODE)]);
