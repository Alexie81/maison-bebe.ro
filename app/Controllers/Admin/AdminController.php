...
final class AdminController extends Controller
{
    ...
    public function coupons(Request $request):string{$items=Database::connection()->query('SELECT * FROM coupons ORDER BY created_at DESC')->fetchAll();return $this->admin('admin/coupons',compact('items'));}
    public function saveCoupon(Request $request):never{$code=mb_strtoupper(trim((string)$request->input('code','')));$type=(string)$request->input('discount_type','percent');$value=(float)$request->input('discount_value',0);$active=$request->input('is_active')?1:0;if($code===''||!in_array($type,['percent','fixed'],true)||$value<=0){throw new HttpException(422,'Datele cuponului nu sunt valide.');}$storedValue=$type==='fixed'?(int)round($value*100):(int)round($value);$minimum=(int)round(((float)$request->input('minimum_order',0))*100);Database::connection()->prepare('INSERT INTO coupons (code,discount_type,discount_value,minimum_order_minor,is_active) VALUES (?,?,?,?,?) ON DUPLICATE KEY UPDATE discount_type=VALUES(discount_type),discount_value=VALUES(discount_value),minimum_order_minor=VALUES(minimum_order_minor),is_active=VALUES(is_active)')->execute([$code,$type,$storedValue,$minimum,$active]);Response::redirect('/admin/cupoane');}
