...
final class AuthController extends Controller
{
    ...
    public function sendReset(Request $request): never
    {
        ...
        if($id){$token=bin2hex(random_bytes(32));$hash=hash('sha256',$token);$pdo=Database::connection();$pdo->beginTransaction();$pdo->prepare('UPDATE password_resets SET used_at=NOW() WHERE user_id=? AND used_at IS NULL')->execute([$id]);$pdo->prepare('INSERT INTO password_resets (user_id,token_hash,expires_at) VALUES (?,?,DATE_ADD(NOW(),INTERVAL 60 MINUTE))')->execute([$id,$hash]);$pdo->prepare("INSERT INTO email_queue (template_key,recipient,subject,payload_json,status,next_attempt_at,correlation_id) VALUES ('password_reset',?,'Recuperează parola Maison Bébé',?,'pending',NOW(),?)")->execute([$email,json_encode(['reset_url'=>absolute_url('/cont/parola-noua/'.$token)],JSON_UNESCAPED_SLASHES|JSON_UNESCAPED_UNICODE),'password-reset:'.$id.':'.substr($hash,0,16)]);$pdo->commit();}
    ...
    {
        $row=$this->resetRow($token);$password=(string)$request->input('password','');$confirmation=(string)$request->input('password_confirmation','');if(!$row||strlen($password)<12||!hash_equals($password,$confirmation)){Session::flash('auth_error','Link invalid, parolele nu coincid sau parola are mai puțin de 12 caractere.');Response::redirect('/cont/parola-noua/'.$token);}
    ...
    {
        $state=(string)$request->input('state','');if($state===''||!hash_equals((string)Session::get('google_oauth_state',''),$state)){throw new HttpException(400,'Starea OAuth nu este validă.');}Session::forget('google_oauth_state');$code=(string)$request->input('code','');if($code===''){Session::flash('auth_error','Autentificarea Google a fost anulată.');Response::redirect('/cont/autentificare');}
