...
final class StorefrontController extends Controller
{
    ...
    public function product(Request $request, string $slug): string
    {
        ...
        $personalizationOptions = $personalizationService->forProduct((int) $product['id'], true);
        $personalizationSettings = $personalizationService->settingsForProduct((int) $product['id']);
        $giftBoxTemplates = [];
        if ($productSet) {
            $totalAvailable = 0;
