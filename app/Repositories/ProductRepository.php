...
final class ProductRepository
{
    ...
    {
        return Database::connection()->query("SELECT c.*,m.path image_path,(SELECT COUNT(*) FROM collection_products cp JOIN products p ON p.id=cp.product_id WHERE cp.collection_id=c.id AND p.status='active' AND p.deleted_at IS NULL) product_count FROM collections c LEFT JOIN media_assets m ON m.id=c.image_id WHERE c.is_active=1 AND c.deleted_at IS NULL ORDER BY c.is_featured DESC,c.sort_order,c.name")->fetchAll();
    }
