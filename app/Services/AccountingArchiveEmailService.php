...
final class AccountingArchiveEmailService
{
    ...
    public function send(
        ...
        $contents = implode(' și ', array_filter([
            $includeNirs ? 'NIR-urile cu facturile furnizorilor' : null,
            $includeStocks ? 'situația Stocuri Conta' : null,
        ]));
        ...
        $archive = (new NirArchiveService())->exportPeriod(
            $from,
            $to,
            $includeNirs,
            $stock['binary'] ?? null,
        ...
        if (strlen($archive['binary']) > self::MAX_EMAIL_ARCHIVE_BYTES) {
            throw new RuntimeException('Arhiva depășește 18 MB și nu poate fi trimisă în siguranță prin email. Selectează o perioadă mai scurtă.');
        }
        ...
        $html = '<!doctype html><html lang="ro"><head><meta name="viewport" content="width=device-width,initial-scale=1"></head>'
            ...
            . '<h1 style="margin:0 0 14px;font:500 27px Georgia,serif">Documente contabile</h1>'
            ...
            . '<p style="margin:22px 0 0;padding:14px;background:#f7f0e9;border-radius:10px">Arhiva ZIP este atașată acestui mesaj.</p>'
            . '</div></div></body></html>';
        ...
        Database::connection()->prepare(
            ...
            json_encode([
                ...
                'includes_stocks' => $includeStocks,
                'document_count' => $archive['count'],
                'sha256' => hash('sha256', $archive['binary']),
            ], JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES),
        ]);

        return ['filename' => $archive['filename'], 'recipient' => $recipient, 'count' => $archive['count']];
    }
}
