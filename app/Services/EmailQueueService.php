...
final class EmailQueueService
{
    ...
    private function render(string $template, array $data, string $subject): string
    {
        ...
        $body = match ($template) {
            ...
            'contact_admin' => '<p style="margin:0 0 8px;color:#9a735d;font-size:12px;font-weight:700;letter-spacing:2px;text-transform:uppercase">Mesaj nou din website</p><h1 style="font-family:Georgia,serif;margin:0 0 22px">' . htmlspecialchars((string) ($data['subject'] ?? 'Mesaj de contact'), ENT_QUOTES, 'UTF-8') . '</h1><div style="padding:18px;background:#f7f1eb;border:1px solid #e4d6ca;border-radius:12px"><p style="margin:0 0 7px"><strong>' . htmlspecialchars((string) ($data['name'] ?? ''), ENT_QUOTES, 'UTF-8') . '</strong></p><p style="margin:0 0 7px"><a href="mailto:' . htmlspecialchars((string) ($data['email'] ?? ''), ENT_QUOTES, 'UTF-8') . '">' . htmlspecialchars((string) ($data['email'] ?? ''), ENT_QUOTES, 'UTF-8') . '</a></p>' . (!empty($data['phone']) ? '<p style="margin:0"><a href="tel:' . htmlspecialchars((string) $data['phone'], ENT_QUOTES, 'UTF-8') . '">' . htmlspecialchars((string) $data['phone'], ENT_QUOTES, 'UTF-8') . '</a></p>' : '') . '</div><div style="margin-top:20px;padding:20px;border-left:3px solid #9a735d;background:#fff"><p style="margin:0;line-height:1.7">' . nl2br(htmlspecialchars((string) ($data['message'] ?? ''), ENT_QUOTES, 'UTF-8')) . '</p></div>',
