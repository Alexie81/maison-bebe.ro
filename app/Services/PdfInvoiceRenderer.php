...
final class PdfInvoiceRenderer {
 ...
 public function render(array $i,array $items,string $path):void{
  ...
  $badge=$isStorno?'STORNARE INTEGRALA':($paid?'ACHITATA':'NEACHITATA');$badgeColor=$isStorno?[166,91,75]:($paid?[72,120,88]:[166,91,75]);$this->fill($c,$badgeColor);$this->rect($c,412,711,139,27,true);$this->text($c,535,720,8,$badge,true,[255,255,255],'right');
  ...
  $t=max(80,$y-15);$this->fill($c,$accent);$this->rect($c,326,$t-75,225,76,true);$this->text($c,445,$t-18,9,'Subtotal fara TVA',false,$dark,'right');$this->text($c,538,$t-18,9,$this->money((int)$i['subtotal_minor']),true,$dark,'right');
  if((int)$i['discount_minor']!==0){$this->text($c,445,$t-35,9,'Reducere fara TVA',false,$dark,'right');$this->text($c,538,$t-35,9,$isStorno?$this->money((int)$i['discount_minor']):'-'.$this->money((int)$i['discount_minor']),true,$dark,'right');}
  $this->text($c,445,$t-50,9,'TVA',false,$dark,'right');$this->text($c,538,$t-50,9,$this->money((int)$i['vat_minor']),true,$dark,'right');$this->stroke($c,$primary);$this->line($c,338,$t-57,539,$t-57);$this->text($c,445,$t-72,11,$isStorno?'TOTAL STORNAT':'TOTAL CU TVA',true,$dark,'right');$this->text($c,538,$t-72,12,$this->money((int)$i['grand_total_minor']),true,$primary,'right');
 ...
 private function ascii(string $v):string{$v=strtr($v,['ă'=>'a','Ă'=>'A','â'=>'a','Â'=>'A','î'=>'i','Î'=>'I','ș'=>'s','Ș'=>'S','ş'=>'s','Ş'=>'S','ț'=>'t','Ț'=>'T','ţ'=>'t','Ţ'=>'T']);return iconv('UTF-8','ASCII//TRANSLIT//IGNORE',$v)?:preg_replace('/[^\x20-\x7E]/','',$v)?:'';}private function escape(string $v):string{return str_replace(['\\','(',')'],['\\\\','\\(','\\)'],$v);}
