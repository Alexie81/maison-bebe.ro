                    ...
                    <div class="address-grid">
                        <?php foreach ($addresses as $address):
                            $addressPayload = json_encode([
                                'name'=>$address['name'],
                                'contact_first_name'=>$address['contact_first_name'] ?: ($profile['first_name'] ?? ''),
                                'contact_last_name'=>$address['contact_last_name'] ?: ($profile['last_name'] ?? ''),
                                'phone'=>$address['phone'] ?? '',
                                'line1'=>$address['line1'],
                                'line2'=>$address['line2'] ?? '',
                                'city'=>$address['city'],
                        ...
                        ?>
                            <article class="address-card">
                                ...
                                <strong><?= e(trim(($address['contact_first_name'] ?: ($profile['first_name'] ?? '')).' '.($address['contact_last_name'] ?: ($profile['last_name'] ?? '')))) ?></strong>
...
<div class="modal-layer address-modal-layer" id="address-modal" role="dialog" aria-modal="true" aria-labelledby="address-modal-title" hidden>
    ...
    <section class="modal-panel address-modal" tabindex="-1">
        ...
        <form method="post" action="<?= e(url('/cont/adrese')) ?>" data-address-form data-create-action="<?= e(url('/cont/adrese')) ?>" data-defaults="<?= e(json_encode(['contact_first_name'=>$profile['first_name']??'','contact_last_name'=>$profile['last_name']??'','phone'=>$profile['phone']??''],JSON_UNESCAPED_UNICODE)) ?>">
            <?= csrf_field() ?>
            <div class="form-grid">
                <label>Prenume persoană de contact<input name="contact_first_name" required value="<?= e($profile['first_name'] ?? '') ?>" autocomplete="given-name"></label>
                <label>Nume persoană de contact<input name="contact_last_name" required value="<?= e($profile['last_name'] ?? '') ?>" autocomplete="family-name"></label>
                <label>Etichetă adresă<input name="name" required placeholder="Acasă"></label>
                <label>Telefon<input name="phone" type="tel" inputmode="tel" autocomplete="tel" value="<?= e($profile['phone'] ?? '') ?>"></label>
