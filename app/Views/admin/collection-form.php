...
<label class="wide">Canonical URL<input type="url" name="canonical_url" value="<?= e($collection['canonical_url']??'') ?>" placeholder="gol = URL-ul propriu al colecției"><small class="help">Pentru o pagină duplicată, indică aici pagina principală care trebuie indexată.</small></label>
...
<aside><section class="admin-panel"><h2>Publicare</h2><label class="check-label"><input type="checkbox" name="is_active" value="1" <?= !isset($collection['is_active'])||$collection['is_active']?'checked':'' ?>> Vizibilă pe website</label><label class="check-label"><input type="checkbox" name="is_indexable" value="1" <?= !isset($collection['is_indexable'])||$collection['is_indexable']?'checked':'' ?>> Permite indexarea în Google</label><p class="help">Dezactivează indexarea când colecția dublează o categorie și completează URL-ul canonical.</p><button class="admin-button" type="submit">Salvează colecția</button></section>
