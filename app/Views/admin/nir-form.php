...
<form class="nir-editor nir-editor-compact" method="post" enctype="multipart/form-data" action="<?= e(url($editing?'/admin/nir-uri/'.$document['id']:'/admin/nir-uri')) ?>" data-nir-editor data-exchange-rate-url="<?= e(url('/admin/nir-uri/curs-valutar')) ?>" data-has-saved-exchange-rate="<?= $editing?'1':'0' ?>">
  ...
  <section id="nir-document" class="admin-panel nir-section nir-document-panel">
    ...
    <div class="accounting-subpanel-row">
      ...
      <details class="accounting-subpanel"><summary><span><b>Atașamente</b><small>Factura, fotografiile paginilor și avizul</small></span><i>+</i></summary><div class="nir-attachments accounting-subpanel-content"><label>Factura PDF<input type="file" name="source_pdf" accept="application/pdf,.pdf"></label><label>Pozele facturii (mai multe pagini)<input type="file" name="source_images[]" accept="image/jpeg,image/png,image/webp,.jpg,.jpeg,.png,.webp" multiple><small class="accounting-field-help">Poți selecta simultan până la 12 imagini JPG, PNG sau WEBP, maximum 12 MB fiecare.</small></label><label>Factura XLSX<input type="file" name="source_xlsx" accept=".xlsx"></label><label>Factura XML<input type="file" name="source_xml" accept="application/xml,text/xml,.xml"></label><label>Aviz PDF<input type="file" name="delivery_note_attachment" accept="application/pdf,.pdf"></label></div></details>
    </div>
  </section>
