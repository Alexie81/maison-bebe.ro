        ...
        <section class="admin-panel">
            ...
            <?php foreach ($items as $item): $itemOptions=json_decode((string)($item['options_json']??''),true)?:[];$itemCustomization=json_decode((string)($item['customization_json']??''),true)?:[];$giftBoxRole=(string)($itemCustomization['role']??'');$optionalChoices=(array)($itemCustomization['optional_variants']??[]);$personalization=(array)($itemCustomization['personalization']??[]);$personalizationChoices=(array)($personalization['options']??[]); ?>
                <div class="admin-order-item<?= $giftBoxRole==='box'?' is-gift-box-line':'' ?>">
                    <div>
                        <strong><?= e($giftBoxRole==='box'?($itemCustomization['template_name']??$item['name_snapshot']):$item['name_snapshot']) ?></strong>
                        <small><?= e($item['sku_snapshot']) ?> · <?= (int) $item['quantity'] ?> buc.</small>
                        <div class="admin-order-item-specs">
                            <?php if(!empty($itemOptions['label'])&&$itemOptions['label']!=='Standard'): ?><span>Variantă: <b><?= e($itemOptions['label']) ?></b></span><?php endif; ?>
                            <?php if($giftBoxRole==='box'): ?><span class="gift">Împachetare: <b><?= e($itemCustomization['template_name']??$item['name_snapshot']) ?></b></span><?php elseif($giftBoxRole==='component'): ?><span class="gift">Cutie cadou: <b><?= e($itemCustomization['template_name']??'Selectată') ?></b></span><?php endif; ?>
                            <?php if(!empty($itemCustomization['recipient_name'])): ?><span>Destinatar: <b><?= e($itemCustomization['recipient_name']) ?></b></span><?php endif; ?>
                            <?php if(!empty($itemCustomization['gift_message'])): ?><span>Mesaj în cutie: <b>„<?= e($itemCustomization['gift_message']) ?>”</b></span><?php endif; ?>
                            <?php foreach($optionalChoices as $optionalChoice): ?><span>Opțiune: <b><?= e($optionalChoice['name']??'') ?><?= (int)($optionalChoice['price_delta_minor']??0)>0?' (+ '.money((int)$optionalChoice['price_delta_minor']).')':'' ?></b></span><?php endforeach; ?>
                            <?php if($personalization): ?>
                                ...
                                <span class="personalization"><?= e($personalization['child_name_label']??'Nume copil') ?>: <b><?= e($personalization['child_name']??'') ?></b></span>
                                <span class="personalization"><?= e($personalization['event_date_label']??'Data botezului/nașterii') ?>: <b><?= e($personalization['event_date_formatted']??$personalization['birth_date_formatted']??'') ?></b></span>
                            <?php endif; ?>
                        </div>
                        <?php if($personalization): ?><div class="admin-personalization-instructions"><small>MESAJ PENTRU ATELIER</small><strong><?= e($personalization['instructions']??'') ?></strong></div><?php endif; ?>
                    </div>
