...
<div>
    ...
    <section class="admin-panel product-personalization-panel" data-personalization-options-panel>
        <div class="panel-head"><div><p class="eyebrow">PERSONALIZARE</p><h2>Opțiuni personalizate pentru produs</h2><p class="help">Configurează tipurile de personalizare, costurile și denumirile câmpurilor pe care le completează clienta.</p></div><button type="button" class="admin-button secondary" data-add-personalization-option>+ Adaugă personalizare</button></div>
        <div class="personalization-admin-note"><span aria-hidden="true">Aa</span><p><strong>Textul este configurabil pentru fiecare produs</strong><small>Produsele existente își păstrează formularea actuală. Pentru tăvițe este folosit automat câmpul „Data evenimentului”.</small></p></div>
        <div class="personalization-field-settings" data-personalization-label-settings>
            <label><span>Eticheta câmpului pentru nume</span><input name="personalization_child_name_label" maxlength="120" value="<?= e($personalizationSettings['child_name_label'] ?? 'Numele copilului') ?>" placeholder="Ex: Numele copilului" data-personalization-name-label-input></label>
            <label><span>Eticheta câmpului pentru dată</span><input name="personalization_event_date_label" maxlength="120" value="<?= e($personalizationSettings['event_date_label'] ?? 'Data evenimentului') ?>" placeholder="Ex: Data evenimentului" data-personalization-date-label-input></label>
            <label class="wide"><span>Text explicativ afișat clientei</span><input name="personalization_helper_text" maxlength="255" value="<?= e($personalizationSettings['helper_text'] ?? '') ?>" placeholder="Ex: Numele copilului și data evenimentului se completează o singură dată mai jos" data-personalization-helper-input></label>
        </div>
        <div class="product-personalization-option-list" data-personalization-option-list>
            <?php foreach($personalizationOptions as $personalizationOption): ?><article class="product-personalization-option-row" data-personalization-option-row>
                ...
                <label><span>Denumirea personalizării</span><input name="personalization_option_name[]" value="<?= e($personalizationOption['name']) ?>" placeholder="Ex: Nume și dată"></label>
