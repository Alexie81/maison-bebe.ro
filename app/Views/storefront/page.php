...
<section class="about-finale"><div class="home-shell"><p class="eyebrow">PENTRU FIECARE NOU ÎNCEPUT</p><h2>Un dar ales cu inimă.</h2><p><?= !empty($hasActiveGiftBox) ? 'Descoperă Gift Box-uri, hăinuțe și accesorii pregătite să devină amintiri.' : 'Descoperă hăinuțe și accesorii pregătite să devină amintiri.' ?></p><div class="button-row"><?php if (!empty($hasActiveGiftBox)): ?><a class="button" href="<?= e(url('/gift-box')) ?>">Alege un Gift Box</a><?php else: ?><a class="button" href="<?= e(url('/shop')) ?>">Descoperă colecția</a><?php endif; ?><?php if (!empty($hasPublishedArticles)): ?><a class="button button-outline" href="<?= e(url('/atelier')) ?>">Povești din Atelier</a><?php endif; ?></div></div></section>
<?php else: ?>
<section class="page-hero shell section-space-small"><p class="eyebrow">Maison Bébé</p><h1><?= e($page['title'] ?? 'Maison Bébé') ?></h1></section>
<section class="legal-layout shell section-space-small"><aside aria-label="Politici Maison Bébé"><a href="<?= e(url('/politici/livrare-si-retur')) ?>">Livrare și retur</a><a href="<?= e(url('/politici/termeni-si-conditii')) ?>">Termeni și condiții</a><a href="<?= e(url('/politici/confidentialitate')) ?>">Confidențialitate</a><a href="<?= e(url('/politici/cookies')) ?>">Cookie-uri</a></aside><article class="prose legal-prose"><?= $page['content_html'] ?? '' ?></article></section>
<?php endif; ?>
