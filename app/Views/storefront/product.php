    ...
    <div class="product-layout">
        ...
        <div class="product-summary">
            ...
            <form data-add-to-cart-form data-ga4-product-detail data-ga4-item="<?= e(json_encode($ga4ProductItem, JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES)) ?>">
                ...
                <?php foreach ($product['options'] as $option): $singleOptionValue=count($option['values'])===1; ?><fieldset class="variant-options" data-option><legend><?= e($option['name']) ?></legend><div><?php foreach ($option['values'] as $value): ?><button type="button" class="<?= $singleOptionValue?'active':'' ?>" data-option-value="<?= (int)$value['value_id'] ?>"<?= $singleOptionValue?' data-fixed-option="1" aria-disabled="true"':'' ?> aria-pressed="<?= $singleOptionValue?'true':'false' ?>"><?= e($value['value']) ?></button><?php endforeach; ?></div></fieldset><?php endforeach; ?>
                ...
                <?php if(!empty($personalizationOptions)): $personalizationNameLabel=(string)($personalizationSettings['child_name_label']??'Numele copilului');$personalizationDateLabel=(string)($personalizationSettings['event_date_label']??'Data botezului/nașterii');$personalizationHelper=(string)($personalizationSettings['helper_text']??''); ?>
                    <fieldset class="product-personalization-choices" data-product-personalization data-personalization-name-label="<?= e($personalizationNameLabel) ?>" data-personalization-date-label="<?= e($personalizationDateLabel) ?>">
                        <legend><span>Personalizează produsul</span><small>Opțional</small></legend>
                        <p class="personalization-choice-intro">Poți alege una sau mai multe personalizări. Datele pentru atelier se completează o singură dată.</p>
                        <div class="personalization-choice-list">
                            <?php foreach($personalizationOptions as $personalizationOption): $personalizationPrice=(int)$personalizationOption['price_delta_minor']; ?>
                                <label><input type="checkbox" name="personalization_option_ids[]" value="<?= (int)$personalizationOption['id'] ?>" data-personalization-option data-extra-price="<?= $personalizationPrice ?>"><span class="personalization-choice-check" aria-hidden="true"></span><span class="personalization-choice-copy"><strong><?= e($personalizationOption['name']) ?></strong><small><?= e($personalizationHelper) ?></small></span><?php if($personalizationPrice>0): ?><b class="personalization-choice-price">+ <?= money($personalizationPrice) ?></b><?php endif; ?></label>
                            <?php endforeach; ?>
                        </div>
                        <div class="personalization-details" data-personalization-details hidden>
                            <div class="personalization-details-head"><span aria-hidden="true">✦</span><p><strong>Datele pentru atelier</strong><small>Aceste date se aplică tuturor personalizărilor bifate.</small></p></div>
                            <div><label><?= e($personalizationNameLabel) ?> *<input name="personalization_child_name" maxlength="120" autocomplete="off" placeholder="Ex: Sofia Maria" data-personalization-child-name></label><label><?= e($personalizationDateLabel) ?> *<input type="date" name="personalization_birth_date" min="1900-01-01" data-personalization-birth-date></label></div>
                            <p class="personalization-preview" data-personalization-preview>Mesajul pentru personalizare va apărea aici.</p>
                        </div>
                    </fieldset>
                <?php endif; ?>
                <?php if(!empty($productGiftBoxOffer['allow_gift_box']) && !empty($giftBoxTemplates)): ?><fieldset class="product-gift-box-packaging"><legend><span>Împachetare în cutie cadou</span><small>Opțional</small></legend><p class="gift-box-packaging-intro">Alege dacă dorești ca produsul să fie pregătit în cutia cadou.</p><div class="gift-box-packaging-list"><label><input type="radio" name="gift_box_template_id" value="0" data-box-price="0" checked><span class="gift-box-choice-check" aria-hidden="true"></span><span class="gift-box-choice-copy"><strong>Fără cutie cadou</strong><small>Prețul produsului rămâne neschimbat</small></span></label><?php foreach($giftBoxTemplates as $boxTemplate): $boxAvailable=!(int)($boxTemplate['track_inventory']??1)||(int)($boxTemplate['stock_qty']??0)>0; ?><label class="<?= $boxAvailable?'':'is-unavailable' ?>"><input type="radio" name="gift_box_template_id" value="<?= (int)$boxTemplate['id'] ?>" data-box-price="<?= (int)$boxTemplate['price_minor'] ?>" <?= $boxAvailable?'':'disabled' ?>><span class="gift-box-choice-check" aria-hidden="true"></span><img src="<?= e(url($boxTemplate['image_path'])) ?>" alt="<?= e($boxTemplate['name']) ?>"><span class="gift-box-choice-copy"><strong><?= e($boxTemplate['name']) ?></strong><small><?= $boxAvailable?'Împachetare cadou pentru acest produs':'Cutia este indisponibilă momentan' ?></small></span><b class="gift-box-choice-price"><?= $boxAvailable?'+ '.money((int)$boxTemplate['price_minor']):'Indisponibilă' ?></b></label><?php endforeach; ?></div></fieldset><?php endif; ?>
