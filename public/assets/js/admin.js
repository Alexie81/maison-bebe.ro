(() => {
  ...
  const normalizeAdminProductSearch=value=>String(value||'').normalize('NFD').replace(/[\u0300-\u036f]/g,'').replace(/[ăâ]/gi,'a').replace(/î/gi,'i').replace(/[șş]/gi,'s').replace(/[țţ]/gi,'t').toLowerCase().replace(/[^a-z0-9]+/g,' ').trim().replace(/\s+/g,' ');
  const adminProductSearchStem=value=>{
    const term=normalizeAdminProductSearch(value);
    for(const suffix of ['urilor','iilor','elor','ilor','ului','ri'])if(term.length-suffix.length>=4&&term.endsWith(suffix))return term.slice(0,-suffix.length);
  ...
  const adminProductEditDistance=(left,right)=>{
    ...
    for(let leftIndex=1;leftIndex<=left.length;leftIndex++){
      ...
      for(let rightIndex=1;rightIndex<=right.length;rightIndex++){
        const saved=row[rightIndex];row[rightIndex]=Math.min(row[rightIndex]+1,row[rightIndex-1]+1,previous+(left[leftIndex-1]===right[rightIndex-1]?0:1));previous=saved;
      }
    }
    return row[right.length];
  };
  const adminProductAlternatives=term=>{
  ...
  const updateAdminProductSearch=widget=>{
    const input=widget?.querySelector('[data-admin-product-search-input]');
    const popup=widget?.querySelector('[data-admin-product-search-popup]');
    const target=widget?.querySelector('[data-admin-product-search-results]');
    if(!input||!popup||!target)return;
    const query=normalizeAdminProductSearch(input.value);
    ...
    matches.forEach(({card})=>{card.hidden=false;target.append(card);});
    const empty=widget.querySelector('[data-admin-product-search-empty]');if(empty)empty.hidden=matches.length>0;
    const count=widget.querySelector('[data-admin-product-search-count]');if(count)count.textContent=matches.length?`${matches.length} sugestii`:'0 rezultate';
    const heading=widget.querySelector('[data-admin-product-search-heading]');if(heading)heading.textContent=query?'Produse potrivite':'Produse recente';
    popup.hidden=false;input.setAttribute('aria-expanded','true');input.removeAttribute('aria-activedescendant');
  };
  document.addEventListener('input',event=>{const input=event.target.closest('[data-admin-product-search-input]');if(input)updateAdminProductSearch(input.closest('[data-product-catalog-search]'));});
  document.addEventListener('focusin',event=>{const input=event.target.closest('[data-admin-product-search-input]');if(input){closeAdminProductSearches(input.closest('[data-product-catalog-search]')?.querySelector('[data-admin-product-search-popup]'));updateAdminProductSearch(input.closest('[data-product-catalog-search]'));}});
  document.addEventListener('keydown',event=>{
    const input=event.target.closest('[data-admin-product-search-input]');if(!input)return;
    const widget=input.closest('[data-product-catalog-search]');const popup=widget?.querySelector('[data-admin-product-search-popup]');
    if(event.key==='Escape'){if(popup)popup.hidden=true;input.setAttribute('aria-expanded','false');return;}
    if(!['ArrowDown','ArrowUp','Enter'].includes(event.key)||!popup||popup.hidden)return;
    const visible=[...widget.querySelectorAll('[data-admin-product-search-result]:not([hidden])')];let active=visible.findIndex(card=>card.classList.contains('is-active'));
    if(!visible.length)return;
    if(event.key==='Enter'){if(active>=0){event.preventDefault();visible[active].click();}return;}
    event.preventDefault();active=event.key==='ArrowDown'?(active+1)%visible.length:(active<=0?visible.length-1:active-1);
    visible.forEach((card,index)=>{const selected=index===active;card.classList.toggle('is-active',selected);card.setAttribute('aria-selected',String(selected));});visible[active]?.scrollIntoView({block:'nearest'});
  });
  document.addEventListener('click',event=>{
    const result=event.target.closest('[data-admin-product-search-result]');
    if(result){const widget=result.closest('[data-product-catalog-search]');const input=widget?.querySelector('[data-admin-product-search-input]');if(input){input.value=result.dataset.name||'';widget.querySelector('[data-admin-product-search-popup]').hidden=true;widget.querySelector('form')?.requestSubmit();}return;}
    if(!event.target.closest('[data-product-catalog-search]'))closeAdminProductSearches();
  });
  const enhancePartialContent=root=>{
    ...
    root.querySelectorAll('.admin-panel [required]').forEach(control=>{
      ...
      const anchor=control.parentElement===label?control:control.parentElement;
      if(anchor&&anchor.parentElement===label)label.insertBefore(tag,anchor);else label.append(tag);
    });
  ...
  document.addEventListener('input',event=>{
    ...
    if(!control)return;
    if(control.closest('[data-admin-ignore-dirty]'))return;
  ...
  document.addEventListener('change',event=>{
    ...
    if(control&&!control.closest('[data-admin-ignore-dirty]'))markAdminDirty(control.closest('form'));
  });
  ...
  const adminListUrl=action=>{
    ...
    const orderAction=path.match(/^\/admin\/comenzi\/(\d+)\/(?:status|plata-ramburs|factura)$/);
    if(orderAction)return new URL(base+'/admin/comenzi/'+orderAction[1],location.origin).href;
    const routes=[
      ...
      [/^\/admin\/cms\/homepage(?:\/|$)/,'/admin/cms/homepage'],
      [/^\/admin\/cms\/pagini(?:\/|$)/,'/admin/cms'],
      [/^\/admin\/comenzi(?:\/|$)/,'/admin/comenzi'],
      [/^\/admin\/facturi(?:\/|$)/,'/admin/facturi'],
      [/^\/admin\/nir-uri(?:\/|$)/,'/admin/nir-uri'],
      [/^\/admin\/setari\/livrare(?:\/|$)/,'/admin/setari/livrare'],
  ...
  if(productEditor){
    ...
    const productSetToggle=productEditor.querySelector('[data-product-set-toggle]');
    const giftBoxProductToggle=productEditor.querySelector('[data-gift-box-product-toggle]');
    const giftBoxDimensions=productEditor.querySelector('[data-gift-box-dimensions]');
    const productSetPanel=productEditor.querySelector('[data-product-set-panel]');
    const productSetCount=productEditor.querySelector('[data-product-set-count]');
    const setGiftBoxToggle=productEditor.querySelector('[data-set-gift-box-toggle]');
    ...
    const syncOptionalVariantEmpty=()=>{if(optionalVariantEmpty)optionalVariantEmpty.hidden=Boolean(optionalVariantList?.querySelector('[data-optional-variant-row]'));};
    ...
    const syncProductSetComponents=()=>{
      ...
      rows.forEach(row=>{const toggle=row.querySelector('[data-product-set-component-toggle]');const quantity=row.querySelector('[data-product-set-component-quantity]');const checked=Boolean(toggle?.checked);row.classList.toggle('is-selected',checked);if(quantity)quantity.disabled=!checked;if(checked)selected++;});
      if(productSetCount)productSetCount.textContent=String(selected);
    };
    const syncProductSetMode=()=>{
    ...
    productSetPanel?.querySelector('[data-product-set-search]')?.addEventListener('input',event=>{
      const normalize=value=>String(value||'').normalize('NFD').replace(/[\u0300-\u036f]/g,'').toLowerCase().trim();const query=normalize(event.target.value);let visible=0;
    ...
    const rowState=row=>{let options={};try{options=syncVariant(row)}catch{}return{id:row.querySelector('[name="variant_id[]"]')?.value||'',sku:row.querySelector('.variant-sku strong')?.textContent?.trim()||'Generat automat',options,price:row.querySelector('[name="variant_price[]"]')?.value||'',ean:row.querySelector('[name="variant_ean[]"]')?.value||'',stock:row.querySelector('[name="variant_stock[]"]')?.value||'0',unlimited:row.querySelector('[data-unlimited-stock]')?.checked||false,accounting:row.querySelector('[data-accounting-stock]')?.checked??true};};
    ...
    const variantRow=state=>{const row=document.createElement('article');row.className='variant-row'+(state.unlimited?' is-unlimited':'');row.dataset.variantRow='';row.innerHTML=`<input type="hidden" name="variant_id[]" value="${escapeAdmin(state.id||'')}"><input type="hidden" name="variant_options_json[]" value="${escapeAdmin(JSON.stringify(state.options||{}))}" data-variant-options-json><div class="variant-identity"><div class="variant-sku"><span>Cod SKU</span><strong>${escapeAdmin(state.sku||'Generat automat')}</strong></div><div class="variant-option-selects" data-variant-option-selects></div></div><label data-variant-price-label><span>Preț variantă (lei)</span><input type="number" step="0.01" min="0" name="variant_price[]" value="${escapeAdmin(state.price||'')}" required></label><label><span>EAN / cod de bare</span><input name="variant_ean[]" value="${escapeAdmin(state.ean||'')}" maxlength="32" placeholder="opțional"></label><div class="variant-stock-control"><input type="hidden" name="variant_unlimited[]" value="${state.unlimited?'1':'0'}" data-unlimited-value><label class="variant-stock-field" data-stock-field><span>Stoc disponibil</span><input type="number" min="0" name="variant_stock[]" value="${escapeAdmin(state.stock||'0')}" ${state.unlimited?'readonly':''} data-stock-input></label><label class="admin-switch-row"><input type="checkbox" ${state.unlimited?'checked':''} data-unlimited-stock><span class="admin-switch" aria-hidden="true"><i></i></span><b>Stoc nelimitat</b></label></div><div class="variant-accounting-control"><input type="hidden" name="variant_accounting[]" value="${state.accounting===false?'0':'1'}" data-accounting-value><label class="admin-switch-row"><input type="checkbox" ${state.accounting===false?'':'checked'} data-accounting-stock><span class="admin-switch" aria-hidden="true"><i></i></span><b>Urmărit în Stocuri Conta</b></label><small>Independent de stocul online</small></div><button type="button" class="icon-action danger" data-remove-variant aria-label="Șterge varianta" title="Șterge varianta"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M4 7h16M9 7V4h6v3m3 0-1 13H7L6 7m4 4v5m4-5v5"/></svg></button>`;renderVariantOptions(row);return row;};
    ...
    variantsTarget?.addEventListener('change',event=>{const accountingToggle=event.target.closest('[data-accounting-stock]');if(accountingToggle){const hidden=accountingToggle.closest('[data-variant-row]')?.querySelector('[data-accounting-value]');if(hidden)hidden.value=accountingToggle.checked?'1':'0';return;}const toggle=event.target.closest('[data-unlimited-stock]');if(!toggle)return;const row=toggle.closest('[data-variant-row]');const hidden=row?.querySelector('[data-unlimited-value]');const stock=row?.querySelector('[data-stock-input]');if(hidden)hidden.value=toggle.checked?'1':'0';if(row)row.classList.toggle('is-unlimited',toggle.checked);if(stock){stock.readOnly=toggle.checked;stock.setAttribute('aria-disabled',toggle.checked?'true':'false');if(toggle.checked)stock.value='0';}});
    syncVariantsToOptions();
