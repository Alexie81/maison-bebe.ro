...
try{
    ...
    $pdo->prepare("INSERT INTO invoice_items (invoice_id,order_item_id,name,sku,quantity,unit_price_minor,total_minor,sort_order) VALUES (?,?,'Personalizare test','PERS-9001',1,100,100,1)")->execute([$salesInvoiceId,$orderItemId]);
    ...
    $count->execute([$salesInvoiceId]);$assert((int)$count->fetchColumn()===1,'Personalizarea facturată a creat în mod greșit o ieșire suplimentară din stoc.');
    ...
    $assert((string)$document['exchange_rate']==='5.000000'&&(string)$document['exchange_rate_source']==='BNR'&&(string)$document['exchange_rate_date']==='2026-01-01','Cursul istoric introdus manual nu a păstrat sursa și data alese.');
    ...
    $supplierLineStatement=$pdo->prepare('SELECT id FROM supplier_invoice_lines WHERE supplier_invoice_id=? ORDER BY id LIMIT 1');$supplierLineStatement->execute([$invoiceId]);$supplierLineId=(int)$supplierLineStatement->fetchColumn();
    $pdo->prepare('UPDATE supplier_invoice_lines SET value_without_vat=0,vat_value=0,total_with_vat=0 WHERE id=?')->execute([$supplierLineId]);
    $resaveInput=$input;$resaveInput['row_version']=2;$resaveInput['supplier_line_id']=[$supplierLineId];$service->updateDraft($nirId,$resaveInput);
    $supplierValues=$pdo->prepare('SELECT value_without_vat,vat_value,total_with_vat FROM supplier_invoice_lines WHERE id=?');$supplierValues->execute([$supplierLineId]);$supplierValues=$supplierValues->fetch();
    $resavedDocument=$service->document($nirId);
    $assert((string)$supplierValues['value_without_vat']==='125.00'&&(string)$supplierValues['vat_value']==='23.75'&&(string)$supplierValues['total_with_vat']==='148.75','Resalvarea NIR-ului nu a sincronizat valorile liniei facturii furnizorului.');
    $assert((string)$resavedDocument['total_without_vat']==='125.00'&&(string)$resavedDocument['vat_total']==='23.75'&&(string)$resavedDocument['grand_total']==='148.75','Totalurile facturii furnizorului nu au fost refăcute după resalvarea liniei.');
    ...
    $balance->execute([$variantId,$warehouseId]);$assert(abs((float)$balance->fetch()['current_quantity']+2.0)<0.0001,'Salvarea recepției în ciornă a modificat soldul contabil.');$confirmed=$service->confirm($nirId,['row_version'=>3]);
