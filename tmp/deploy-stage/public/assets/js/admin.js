...
const launchGuide=document.querySelector('[data-launch-guide]');
const launchGuideKey='maison-bebe-admin-launch-guide-hidden';
const setLaunchGuideVisibility=hidden=>{
  if(!launchGuide)return;
  launchGuide.hidden=hidden;
  try{localStorage.setItem(launchGuideKey,hidden?'1':'0');}catch(error){}
};
if(launchGuide){
  ...
  if(forceGuide){try{localStorage.setItem(launchGuideKey,'0');}catch(error){}}
  launchGuide.hidden=hidden;
  launchGuide.querySelector('[data-hide-launch-guide]')?.addEventListener('click',()=>setLaunchGuideVisibility(true));
}