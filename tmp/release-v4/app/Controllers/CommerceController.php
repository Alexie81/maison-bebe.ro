...
final class CommerceController extends Controller
{
    ...
    {
        $payload=$request->all();
        if(!hash_equals((string)Session::get('checkout_idempotency',''),(string)($payload['idempotency_key']??''))){throw new HttpException(419,'Sesiunea checkout-ului a expirat.');}
    ...
    {
        return $this->storefront('storefront/contact',['sent'=>Session::flash('contact_sent'),'meta'=>['title'=>'Contact | Maison Bébé','description'=>'Scrie-ne pentru ajutor cu o comandă sau alegerea unui dar.','canonical'=>absolute_url('/contact')]]);
    }
