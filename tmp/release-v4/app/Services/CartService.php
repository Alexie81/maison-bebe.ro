...
final class CartService
{
    ...
    public function current(): array
    {
        ...
        $statement = $pdo->prepare("SELECT * FROM carts WHERE token=? AND status='active' LIMIT 1");
        $statement->execute([$token]);
        $cart = $statement->fetch();
        if (!$cart) {
            $pdo->prepare("INSERT INTO carts (user_id,token,status) VALUES (?,?,'active')")->execute([Auth::id(), $token]);
            $cart = ['id' => (int) $pdo->lastInsertId(), 'user_id' => Auth::id(), 'token' => $token, 'coupon_code' => null, 'currency' => 'RON', 'status' => 'active'];
        } elseif (Auth::id() && !$cart['user_id']) {
    ...
    public function add(int $variantId, int $quantity = 1, array $customization = []): array
    {
        ...
        return Database::transaction(function (PDO $pdo) use ($variantId, $quantity, $customization): array {
            ...
            if (!$variant || $variant['status'] !== 'active') {
                throw new HttpException(422, 'Varianta nu mai este disponibilă.');
            }
            $cart = $this->current();
            ...
            if ($newQuantity > (int) $variant['stock_qty']) {
                throw new HttpException(422, 'Stoc insuficient pentru cantitatea aleasă.');
            }
            if ($item) {
            ...
            return ['name' => $variant['product_name'], 'variant' => $variant['option_label'] ?: 'Standard', 'quantity' => $quantity, 'slug' => $variant['slug']];
        });
    }
    ...
    public function update(int $itemId, int $quantity): void
    {
        ...
        Database::transaction(static function (PDO $pdo) use ($cart, $itemId, $quantity): void {
            ...
            if (!$item) { throw new HttpException(404, 'Produsul din coș nu există.'); }
            if ($quantity > (int) $item['stock_qty']) { throw new HttpException(422, 'Stoc insuficient.'); }
    ...
    public function applyCoupon(string $code): array
    {
        ...
        if (!$coupon) { throw new HttpException(422, 'Codul promoțional nu este valid.'); }
        $totals = $this->totals();
        if ($totals['subtotal_minor'] < (int) $coupon['minimum_order_minor']) { throw new HttpException(422, 'Valoarea minimă pentru acest cod nu a fost atinsă.'); }
    ...
    public function totals(): array
    {
        ...
        if ($cart['coupon_code']) {
            ...
            if ($coupon && $subtotal >= (int) $coupon['minimum_order_minor']) {
                ...
                if ($coupon['maximum_discount_minor']) { $discount = min($discount, (int) $coupon['maximum_discount_minor']); }
